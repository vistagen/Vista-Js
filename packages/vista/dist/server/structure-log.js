...
exports.logDevBlocked = logDevBlocked;
exports.logDevUnblocked = logDevUnblocked;
exports.logWatcherStart = logWatcherStart;
exports.formatIssuesForOverlay = formatIssuesForOverlay;
exports.formatBuildFailTable = formatBuildFailTable;
// ============================================================================
// ANSI Helpers
// ============================================================================
const RESET = '\x1b[0m';
...
function formatIssue(issue, verbose) {
    ...
    const codeTag = issue.severity === 'error' ? `${RED}${issue.code}${RESET}` : `${YELLOW}${issue.code}${RESET}`;
...
function formatSummary(result) {
    ...
    const parts = [];
    if (errors.length > 0)
        parts.push(`${RED}${errors.length} error(s)${RESET}`);
    if (warnings.length > 0)
        parts.push(`${YELLOW}${warnings.length} warning(s)${RESET}`);
 ...
 */
function logValidationResult(result, logLevel = 'compact') {
 ...
 */
function logDevBlocked() {
    console.log(`${PREFIX} ${RED}${BOLD}Dev server blocked.${RESET} Fix structure errors to continue.`);
}
/**
 * Log when the dev server unblocks after recovery.
 */
function logDevUnblocked() {
    console.log(`${PREFIX} ${GREEN}Structure errors resolved. Dev server unblocked.${RESET}`);
}
/**
 * Log watcher start.
 */
function logWatcherStart() {
    if (process.env.VISTA_DEBUG) {
 ...
 */
function formatIssuesForOverlay(result, includeWarnings = false) {
 ...
 */
function formatBuildFailTable(result) {
    const errors = result.issues.filter((i) => i.severity === 'error');
    if (errors.length === 0)
        return '';
