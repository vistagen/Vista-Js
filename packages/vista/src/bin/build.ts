...
function generateClientEntry(cwd: string, vistaDir: string, clientComponents: Array<{relativePath: string, absolutePath: string}>, isDev: boolean = false) {
    ...
    function serializeRouteStructure(node: any): string {
        ...
        if (node.indexPath) {
            const componentId = path.relative(appDir, node.indexPath).replace(/\\/g, '/').replace(/\.(tsx|ts|jsx|js)$/, '');
            props.push(`indexId: "${componentId}"`);
        }
        if (node.layoutPath) {
            const componentId = path.relative(appDir, node.layoutPath).replace(/\\/g, '/').replace(/\.(tsx|ts|jsx|js)$/, '');
            props.push(`layoutId: "${componentId}"`);
        }
        if (node.loadingPath) {
            const componentId = path.relative(appDir, node.loadingPath).replace(/\\/g, '/').replace(/\.(tsx|ts|jsx|js)$/, '');
            props.push(`loadingId: "${componentId}"`);
        }

        if (node.children && node.children.length > 0) {
            const childrenStr = node.children.map((child: any) => serializeRouteStructure(child)).join(',\n');
            props.push(`children: [\n${childrenStr}\n]`);
        } else {
            props.push(`children: []`);
        }
...
export async function buildClient(watch: boolean = false, onRebuild?: () => void): Promise<webpack.Compiler | null> {
    ...
    if (fs.existsSync(appDir)) {
        console.log(`[Vista] Using ${isNativeAvailable() ? 'Rust native' : 'JS fallback'} scanner (v${getVersion()})`);
    ...
    if (watch) {
        ...
        try {
            chokidar.watch(path.join(cwd, 'app/**/*.css'), { ignoreInitial: true })
                .on('change', () => {
