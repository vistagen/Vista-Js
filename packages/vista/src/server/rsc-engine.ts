...
import {
  ...
  type RSCPayload,
} from '../build/rsc/rsc-renderer';
import type { ClientManifest } from '../build/rsc/client-manifest';
import type { ServerManifest, RouteEntry } from '../build/rsc/server-manifest';
import { initializeRSCModuleSystem, resetRSCState, getClientReferences } from './rsc-module-system';
...
function earlyInitializeRSC(cwd: string) {
  try {
    const clientManifestPath = path.join(cwd, '.vista', 'client-manifest.json');
    if (fs.existsSync(clientManifestPath)) {
      const manifest = JSON.parse(fs.readFileSync(clientManifestPath, 'utf-8'));
      initializeRSCModuleSystem(manifest);
      return manifest;
    }
  } catch (e) {
...
export function startRSCServer(options: RSCEngineOptions = {}): void {
  ...
  let rscRenderer: RSCRenderer | null = null;
  ...
  function loadManifests() {
    try {
      ...
      if (clientManifest && serverManifest) {
        ...
        initializeRSCModuleSystem(clientManifest);

        rscRenderer = new RSCRenderer({
          clientManifest,
          serverManifest,
          cwd,
        });
    ...
    try {
      ...
      if (isDev) {
        ...
        const errorHtml = renderToString(
          React.createElement(ErrorOverlay, { errors: [errorInfo] })
        );
        res
          .status(500)
...
async function renderRSCPage(
  ...
  context: {
    ...
    isDev: boolean;
    vistaConfig: any;
    rscRenderer: RSCRenderer | null;
    clientManifest: ClientManifest | null;
    serverManifest: ServerManifest | null;
  }
): Promise<void> {
 ...
 */
function findAllChunkFiles(cwd: string): string[] {
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.endsWith('.js') && !entry.endsWith('.map')) {
        const stat = fs.statSync(path.join(chunksDir, entry));
...
function generateFullHtml(
  ...
  const hydrationScript = generateHydrationScript(payload);
  const chunkFiles = findAllChunkFiles(cwd);

  // Build script tags for all chunks
  const scriptTags = chunkFiles.map(
    (chunk) => `<script src="/_vista/static/chunks/${chunk}"></script>`
  );
